...
| 能力 | 状态 | 说明 |
| --- | --- | --- |
| v0.5 渐进课程 | Partial | `/training-demo` 已实现七天难度地图、Day 1–4 微课、知识检查、支架练习、独立短答、顺序解锁与刷新恢复；Day 5–7 待实现。 |
...
| 七天渐进课程 | Partial | 七天原创难度与练习契约已建立；Day 1–4 可用，Day 5–7 和最终项目待实现。 |
...
| Beta 发布 | v0.5 development | GitHub Actions、Vercel Preview 和经典静态入口已具备；v0.5 Day 1–4 位于开发分支，真实用户 Pilot 尚未执行。 |
...
Playwright 使用 Mock AI 和本地开发认证，覆盖 v0.5 Day 1–4、顺序解锁、句子排序、理由匹配与循环理由拒绝、刷新恢复、v0.4 完整修订、Demo 无外部 API、登录/Onboarding 和受保护路由跳转。浏览器产物写入 `output/playwright/`，不会提交到 Git。

当前 v0.5 第三批验收基线：33 个 Vitest 文件、462 项单元/组件测试，以及 Desktop Chrome 与移动端 Chromium 共 22 项 Playwright 流程全部通过；静态 Demo 与 Next.js 生产构建通过。规则评测和合成审计报告分别见 [V04_RULE_EVAL](docs/evals/V04_RULE_EVAL.md) 与 [V04_SYNTHETIC_USER_AUDIT](docs/evals/V04_SYNTHETIC_USER_AUDIT.md)。
