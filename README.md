...
Structured Thinking Gym (STG) 是一个面向中文职场用户的结构化表达训练 App。v0.3 首先聚焦临时汇报与工作短答，把“读过方法”变成可观察的主动练习：

```text
无提示回答 → 核心结论自检 → 单点证据反馈 → 亲自重写 → 新题迁移
```

核心定位是：**不是帮用户把这一次写得更好，而是训练用户下一次自己说清楚。** 免费公开训练使用浏览器内确定性规则，不调用付费模型。现有 AI 面试训练后端继续保留为实验能力，不作为 v0.3 用户价值成立的前提。

## 当前状态

| 能力 | 状态 | 说明 |
| --- | --- | --- |
| v0.3 公开训练 | Done | `/training-demo` 覆盖冷回答、单点反馈、主动重写和新题迁移，不调用 OpenAI、Supabase 或 App API。 |
| 三个微技能 | Done | 明确目的、结论先行、两到三点框架；每个技能包含冷题和迁移题。 |
| 透明规则验收 | Done | 三种状态、原文证据、规则版本及 60 个标注回归样本。 |
...
完整定位、竞品边界、实施状态和用户价值验收见 [v0.3 产品策略](docs/V03_PRODUCT_STRATEGY.md)。
...
当前本地验收基线：28 个 Vitest 文件、291 项单元/组件测试；公开训练的 Desktop Chrome 与 Pixel 7 Playwright 完整闭环通过，生产构建通过。
