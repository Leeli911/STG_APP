...
const implementedDays = [1, 2] as const;
...
export function ProgressiveTrainingDemo() {
  ...
  useEffect(() => {
    ...
    const safeSession =
      storedSession.day === 2 &&
      !isProgressiveDayUnlocked(storedProgress, 2)
        ? createProgressiveCourseSession()
        : storedSession;
      ...
      {session.day === 2 && session.stage === "complete" ? (
        ...
        >
          <div className="mt-5 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900">
            Day 3“把结论放到第一句”将在下一批实现。当前页面已展示完整七天难度线，
            但不会把尚未开发的课程伪装成可用功能。
          </div>
        </CompletionCard>
      ) : null}
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {progressiveCourse.map((lesson) => {
          ...
          const interactive =
            lesson.implemented &&
            prerequisiteMet &&
            implementedDays.includes(lesson.day as 1 | 2);
          ...
          return (
            <button
              ...
              key={lesson.id}
              onClick={() => onSelectDay(lesson.day as 1 | 2)}
              type="button"
            >
