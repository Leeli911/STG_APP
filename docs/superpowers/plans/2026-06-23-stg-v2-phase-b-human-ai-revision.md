...
- `200`: first Session creation, same-key replay, or the existing Session for the same initial Attempt. The validated Task 2 Service returns a Session DTO rather than a created/replayed discriminator, so the API does not infer one.
...
- `200`: committed re-score, completed replay, or successful `retry_claimed` re-score from `rescore_failed`
- `202`: identical replay while the Session is already `rescoring`
...
#### 2026-06-25 contract revision

The API preserves the validated Task 2 Service semantics and does not reinterpret database outcomes. A successful `retry_claimed` path completes synchronously and returns `200`; `202` is reserved exclusively for an identical replay whose current Session remains `rescoring`. This revision supersedes any earlier wording that mapped `retry_claimed` itself to `202`.
...
- committed, completed replay, and successful `retry_claimed` return 200
- an identical replay already in `rescoring` returns 202
- `rescore_failed` returns 502 without exposing the underlying AI or persistence error
...
Cover every status and error code in Section 5, including rejecting `feedback_mode`, committed/completed replay/successful retry as 200, already-rescoring replay as 202, 409 different Decision, and 502 with retained Session ID.
...
Follow the existing Attempt API dependency-injection and `jsonSuccess`/`jsonError` patterns. These routes are the authenticated Live transport and use the Supabase repositories so ownership is enforced by both the authenticated user ID and RLS. The later DemoAdapter remains in-memory, makes no API call, and therefore does not require a development API repository.
