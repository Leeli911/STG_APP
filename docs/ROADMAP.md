...
## 当前产品主线 · v0.4 可信评测与间隔训练

Status: **本地实现与验收完成，目标版本 `v0.4.0-beta.1`，待推送发布；真实 Pilot 尚未执行**
...
1. 发布 `v0.4.0-beta.1` 并确认 GitHub Pages 公开入口。
2. 执行 14 天开发者未见题自测，记录耗时、卡点、放弃和遗忘。
3. 新建不参与规则开发的盲测 Fixture，重点覆盖自然口语、隐含请求和无标点表达。
4. 有条件后执行 5–20 人真实 Pilot；真实错误分布优先于合成角色。
5. Pilot 之前不扩展语音、动态 AI 出题、支付或复杂自适应。
