...
## 当前产品主线 · v0.3 结构化表达微训练

Status: **本地实现与验收完成，目标版本 `v0.3.0-beta.1`，待发布与 Pilot**

- 产品从泛化“AI 面试打分”收窄为中文职场短汇报的结构动作训练。
- 首批只做明确目的、结论先行、两到三点框架。
- 主流程固定为无提示冷答、核心结论自检、单点证据反馈、主动重写和新题迁移。
- 公开版完全使用浏览器内确定性规则，不需要付费模型。
- 发版后以 5–20 人 Pilot 验证完成率、即时迁移与 24–72 小时保持，不用自动化测试替代用户价值验证。

详细定位、竞品边界、状态和验收见 [v0.3 产品策略](V03_PRODUCT_STRATEGY.md)。以下 M0–M5 是仓库已有的 AI 面试训练工程主线，继续保留，但不作为 v0.3 免费公开训练的发布前置条件。
