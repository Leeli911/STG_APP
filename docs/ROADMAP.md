...
## 当前产品主线 · v0.5 渐进式学习与毕业项目

Status: **Day 1–7 release candidate 已实现并通过本地质量门，目标版本 `v0.5.0-beta.1`；开发者连续自测待执行**

- 新手入口从“先完成困难冷答”改为“微课、正反例、知识检查、有支架练习、独立表达”。
- 七天难度从识别受众/行动逐步增加到结论、依据、两到三点、完整汇报和毕业项目。
- Day 1 不要求开放文本；Day 2 只写一句明确目的。
- 选择题和支架练习不计为技能达标，独立表达和迁移仍是权威证据。
- Day 1–7 已具备顺序解锁、本地进度、刷新恢复、失败提示和桌面/移动端 E2E。
- Day 3 已实现首句识别、可键盘操作的句子排序、结论先行开放短答和针对性支架。
- Day 4 已实现直接理由识别、结论与依据组合、独立两句话和循环理由拒绝。
- Day 5 已实现分组识别、六张信息卡归类、独立三点短答和重复分点拒绝。
- Day 6 已实现完整结构识别、免重复输入的句块组装、4–6 句独立汇报和结论/分点/事实/行动请求联合判定。
- Day 7 已实现首稿冻结、原稿预填修改、表面修改拦截、第二个未见情境、首次迁移状态冻结和无原文诚实总结。
- 首屏已区分首次开始、无损续训和总结入口，并自动聚焦当前练习。
- Day 7 已区分“流程走完”和“即时迁移达标”；迁移待加强可继续补练，课后选择题自检会生成一项推荐复练。
- `/training-demo/classic` 保留 v0.4 冷答与可信评测闭环作为有基础用户的水平检查。

下一优先级：

1. 执行 7–14 天开发者自测，记录开始时间、完成时间、放弃冲动、反馈可信度、修改次数和隔天保持。
2. 根据真实摩擦建立“合理但被规则拒绝”的盲测样本，优先修题面和规则覆盖，不先扩课。
3. 只有打字负担反复成为 P0/P1 阻断时，再开发可编辑文本、键盘回退和隐私说明完整的语音转文字 Preview。

详细范围见 [v0.5 渐进式学习与项目制训练方案](V05_PROGRESSIVE_LEARNING_PLAN.md) 与 [Day 1–7 用户旅程会审](V05_USER_JOURNEY_REVIEW_2026-08-04.md)。

## 已完成基线 · v0.4 可信评测与间隔训练

Status: **本地实现与验收完成，保留为经典水平检查；真实 Pilot 尚未执行**

- 产品从泛化“AI 面试打分”收窄为中文职场短汇报的结构动作训练。
- 首批只做明确目的、结论先行、两到三点框架。
- 主流程为无提示冷答、核心结论自检、单点证据反馈、主动重写、未见题迁移和 24 小时间隔冷测。
- 公开版完全使用浏览器内确定性规则，不需要付费模型。
- 18 个原创情境覆盖冷题、近迁移、远迁移和延迟题；刷新可以恢复未完成训练。
- 180 条冻结规则回归集、54 条六角色配对合成轨迹和离线报告已进入自动化验收。
- 当前工程证据不能替代用户价值验证；有招募条件后仍以 5–20 人 Pilot 验证理解、完成、迁移、保持和继续使用意愿。

本轮完成：

- [x] 场景目标取代用户自填结论作为评分真值。
- [x] 四状态、任务状态、自检状态和训练完成分离。
- [x] 元评分话术、无请求决定词、重复分点和最小修改防护。
- [x] 180 条规则 Fixture 与 `npm run eval:structured`。
- [x] 六角色严格配对审计与 `npm run audit:synthetic`。
- [x] 24 小时到期冷测、进度状态和完成会话恢复。
- [x] 桌面端/移动端提交、反投机、刷新和冷测 E2E。

详细范围和证据见 [v0.4 可信评测计划](V04_TRUSTWORTHY_EVAL_PLAN.md)、[产品有用性评估](V04_PRODUCT_UTILITY_ASSESSMENT.md)、[规则评测](evals/V04_RULE_EVAL.md) 与 [合成用户审计](evals/V04_SYNTHETIC_USER_AUDIT.md)。以下 M0–M5 是仓库已有的 AI 面试训练工程主线，继续保留，但不作为免费公开训练的发布前置条件。
