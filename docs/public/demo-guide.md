...
2. 完成一次冷答，确认自检只展示原回答中的句子和“没有明确说出”，不要求再次输入。
3. 查看反馈，确认修改框已经带入原稿，且反馈引用回答原文并只给一个修改动作。
4. 不修改原稿或只修改标点，确认不能进入迁移题。
5. 亲自改善目标结构后进入未见题，确认新题不展示方法提示。
6. 刷新未完成页面，确认步骤、选择和回答恢复。
7. 使用到期 Fixture 或等待 24 小时，确认可以进入另一道未见冷测。
8. 在浏览器网络面板确认没有 App API、OpenAI、Supabase 或第三方分析请求。
